...
color_type touch_colors[] = {
  {0, 255, 255},//blue 0,0,255: KEEP
  {0, 255, 255},//made up color 0,60,255
  {0, 255, 255},//made up: 0,97,255: KEEP
  {0, 255, 255},//don't know name-bluish aqua 0,127,255: KEEP
  {0, 255, 255},//made up 40,191,255
  {0, 255, 255}//dodger blue 30,144,255: KEEP
};
int touch_colors_length = 6;

color_type correspondence_colors[] = {
  {0, 255, 0},//lime: KEEP 0,255,0
  {0, 255, 0},//green: YES 0,128,0
  {0, 255, 0},//dark green: KEEP 0,100,0
  {0, 255, 0},//MADE IT UP: KEEP 20,255,40
  {0, 255, 0},//lime green: KEEP 50,205,20
  {0, 255, 0}//forest green: KEEP 34,139,34
};
...
void resting_lights() {
  for (int r = 0; r < rings_length; r++) {
    for (int p = 0; p < NUM_PIXELS_IN_RING; p++) {
      ...
      rings[r].setPixelColor(p, rings[r].Color(60, 60, 60));//all facets half brightness white
...
void proximity_lights() {
  for (int r = 0; r < rings_length; r++) {
    for (int p = 0; p < NUM_PIXELS_IN_RING; p++) {
      ...
      rings[r].setPixelColor(p, rings[r].Color(0, 255, 255));//corn flower blue: 100, 149, 237
      rings[r].show(); // This sends the updated pixel color to the hardware.
    }
  }
}
